...
| USB Bluetooth | USB Orico BT403 or USB UA18 Hoco |
| USB Wifi | Currently, I can't found method to patch this, so can't use it, but it work with the versions < Tahoe |
...
Discord app crash:
  - When you join the voice channel, app discord will be crash, so we can fix that with amdfriend
    ```sh
    sudo amdfriend --in-place --backup --sign --directories ~/Library/Application\ Support/discord/
    ```
# References
